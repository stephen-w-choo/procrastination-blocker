...
export class ClassifierModels {
    ...
    syncModels() {
        ...
        */
        ...
        this.siteDataRepository.procrastinationSiteList.forEach((site) => {
            trainingDataSiteTitles.push({ text: site.title, class: "procrastination" })
            trainingDataSiteDomains.push({ text: site.domain, class: "procrastination"})
        })

        this.siteDataRepository.productiveSiteList.forEach((site) => {
            trainingDataSiteTitles.push({ text: site.title, class: "productive" })
            trainingDataSiteDomains.push({ text: site.domain, class: "productive" })
        })
    ...
    classify(site: SiteData) {
        ...
        return ((titleProbabilities["procrastination"] + domainProbabilities["procrastination"]) / 2) > this.threshold
    }
}
